...
func (h *AdminHandler) GetStats(c *gin.Context) {
	...
	model.DB.Model(&model.User{}).
		Where("is_suspended = ? OR id IN (SELECT user_id FROM gemini_call_logs WHERE created_at >= ? GROUP BY user_id HAVING COUNT(*) >= 20)", true, startOfDay).
		Count(&quotaExceededUsers)
